  ...
  <el-container>
    <el-header>
      <el-row :gutter="20">
        ...
        <el-col :span="20">
          <el-alert title="列出工作流" type="info" />
        </el-col>
      </el-row>
      ...
      <!-- 角色权限列表 -->
      <el-dialog :visible.sync="authority.visible" :title="authority.description" :before-close="handleClose" width="400px">
        <el-tree
          ref="treeList"
          :data="authority.list"
...
export default {
  ...
  methods: {
    getTableData() {
      ...
      //   console.log(result)
      //   _this.tableData = result
      //   _this.page.total = result.length
      //   _this.loading = false
      // })
    },
    currentChange(index) {
    ...
    deleteRoleEntity(data) {
      ...
      if (data.id > 0) {
        _this
          ...
          .then(() => {
            removeRoleById(data.id).then(result => {
              _this.$notify({
                title: '成功',
                message: '删除成功!',
                type: 'success'
    ...
    getRolePermissions(data) {
      ...
      const _this = this
      if (data.id > 0) {
        getRoleById(data.id).then(result => {
          _this.authority.list = result.all
          _this.authority.checkedKeys = result.have
          _this.authority.id = data.id
          _this.authority.name = data.description
