        ...
        <el-card class="box-card" shadow="never">
          ...
          <el-form ref="pmnForm" :rules="rules" :model="pmnForm" label-width="100px">
            <el-form-item label="权限ID">
              <el-input v-model="pmnForm.id" :disabled="true" />
            </el-form-item>

            <el-form-item label="权限标题" prop="name">
              <el-input v-model="pmnForm.name" :disabled="isEdit" clearable />
            </el-form-item>
...
export default {
  ...
  methods: {
    getPermissionTreeData() {
      ...
      const _this = this
      _this.loading = true
      getMenuTree({ filter: false }).then(result => {
    ...
    getMenusTreeData() {
      ...
      const _this = this
      _this.loading = true
      getMenuTree({ filter: true }).then(result => {
    ...
    deleteEntity() {
      const _this = this
      if (_this.pmnForm.id > 0) {
        _this
          .$confirm(
            '确定要删除【' +
              _this.pmnForm.name +
              '】吗? 请确认这个菜单下面没有子菜单.并且没有角色在使用这个菜单.否则无法删除 是否继续?',
            '警告',
            {
          ...
          .then(() => {
            removeMenuById(_this.pmnForm.id).then(result => {
              _this.$notify({
                type: 'success',
                title: '成功',
                message: '删除菜单成功!'
    ...
    saveAndFlush() {
      ...
      _this.$refs.pmnForm.validate(valid => {
        if (valid) {
          if (_this.pmnForm.id > 0) {
            updateMenu(_this.pmnForm).then(result => {
              _this.$notify({
                title: '成功',
                message: '修改菜单成功!',
                type: 'success'
            ...
            saveMenu(_this.pmnForm).then(result => {
              _this.$notify({
                title: '成功',
                message: '新增菜单成功!',
                type: 'success'
